    ...
    const loadAllData = async () => {
      try {
        ...
        const supplierInvoiceItems = supplierInvoices.reduce((acc, row) => {
          if (!row.INVOICE_NUMBER || !row.ITEM_NUMBER_COL) return acc;
          ...
          acc[row.INVOICE_NUMBER].items.push({
            itemNumber: row.ITEM_NUMBER_COL.trim(),
            description: row.DESC_COL.trim(),
            quantity: parseInt(row.QTY_SHIP_COL || '0'),
              ...
              <div key={invoice['Invoice Number']} className="border-b pb-4">
                    ...
                    {invoice.supplierData && (
                      <div className="mt-4 bg-gray-50 p-4 rounded">
                        ...
                        {invoice.supplierData.items.length > 0 ? (
                          <div className="space-y-2">
                            {invoice.supplierData.items.map((item, idx) => (
                        ...
                        <div className="mt-2 pt-2 border-t flex justify-between">
                          ...
                          <span>${invoice.supplierData.totalDue.toFixed(2)}</span>
                        </div>
                      </div>
                    )}
                </div>
